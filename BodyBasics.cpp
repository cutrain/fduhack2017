...
/// </summary>
// DY {
bool saved[BODY_COUNT];
Joint presave_joints[BODY_COUNT][JointType_Count];
D2D1_POINT_2F presave_jointPoints[BODY_COUNT][JointType_Count];
// DY }
...
// DY {
void getBodyPosition(const Joint& joint, float& X, float& Y, float& Z)
{
	const CameraSpacePoint& bodyPoint = joint.Position;
	X = bodyPoint.X;
	Y = bodyPoint.Y;
	Z = bodyPoint.Z;
}
void CBodyBasics::DrawNumber(int num)
{
	D2D1_POINT_2F SP, TP;
	SP.x = 10; SP.y = 10;
	TP.x = 10; TP.y = 100;
	m_pRenderTarget->DrawLine(SP, TP, m_pBrushBoneTracked, c_TrackedBoneThickness);
}
// DY }
                ...
                if (pBody)
                {
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
							...
							if (!saved[i])
							{
								memcpy(presave_jointPoints[i], jointPoints, sizeof(jointPoints));
								memcpy(presave_joints[i], joints, sizeof(joints));
								saved[i] = true;
							}
							PointF lean;
